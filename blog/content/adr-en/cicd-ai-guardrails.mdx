---
title: "Keeping AI in Check — A Safety Net Built with 15 CI/CD Jobs"
date: "2026-04-09"
excerpt: "How 15 CI jobs verify AI-generated code before it reaches production — from secret scanning and test matrices to build gates and GitOps locks, a complete account of the journey."
...
> This post is part 4 of the [AlgoSu tech blog series](/en/posts/agent-orchestration-solo-dev) (6 parts).
> Previous: [Oracle Decides, 11 Agents Execute](/en/posts/orchestration-structure) | Next: [Meet the Agents](/en/posts/meet-the-agents)
...
The key in this diagram is the **direction of the arrows**. Flow only goes from top to bottom, and no job can skip the job above it. `build-services` only runs when `test-node`, `audit-npm`, and `secret-scan` have **all** succeeded. If even one fails, the build itself is blocked.
...
In the AlgoSu project, AI generates the bulk of the code. Twelve agents divide the work by role, and Oracle coordinates the whole operation. But no matter how sophisticated the agent system is, final verification is performed not by humans but by an **automated pipeline**.

The 15 CI jobs ask these questions about AI-generated code:
...
Code reaches production only when all these questions are answered with "yes."

In an era where AI rapidly generates code, I believe the developer's role is shifting from writing code to **designing systems that verify code**. The faster you build, the more thorough the verification must be. And since human review alone has its limits, automated pipelines are essential.

Fifteen jobs may sound like a lot, but if even one of them had been missing, there would have been a production incident. Safety nets work better when they're dense — the tighter the mesh, the fewer things slip through.

> In the next post, we introduce AlgoSu's 12 agents one by one.
> [Meet the Agents](/en/posts/meet-the-agents)

...
tags: ["session", "jwt", "cookie", "debugging", "nestjs"]
order: 7
---
...
We started investigating. Long story short, **4 layers were each running on different clocks.**

> This is post 7 of the [AlgoSu tech blog series](/en/posts/agent-orchestration-solo-dev).
> Previous: [Sprint 3 to 67 — Growth Record of a Solo-Built Service](/en/posts/sprint-journey)

---

## Four Clocks, Four Truths

Visualizing the problem looks like this:

<MetricGrid cols={4}>
  <MetricCard label="JWT TTL" value="7 days" hint="Production env (designed: 2h)" accent={5} />
  <MetricCard label="Cookie maxAge" value="1 hour" hint="Hardcoded constant" accent={5} />
  <MetricCard label="Sliding threshold" value="5 min" hint="Only refreshes within 5 min of expiry" accent={5} />
  <MetricCard label="Frontend timer" value="65 min" hint="Forced logout judgment" accent={5} />
</MetricGrid>
...
<Mermaid
  caption="Before — 4 independent clocks"
  chart={`
flowchart TB
...
The initial plan was to simply replace each hardcoded value with the correct one. But PM feedback came in:
...
<Mermaid
  caption="After — Server env as the sole SSoT"
  chart={`
flowchart TB
    ENV["Server env (SSoT)"]
...
<MetricGrid cols={3}>
  <MetricCard label="JWT_EXPIRES_IN" value="2h" hint="Access Token TTL" accent={1} />
  <MetricCard label="SESSION_REFRESH_THRESHOLD" value="1h" hint="Sliding refresh threshold" accent={2} />
  <MetricCard label="SESSION_HEARTBEAT_INTERVAL" value="10m" hint="Frontend heartbeat interval" accent={3} />
  <MetricCard label="SESSION_TIMEOUT_BUFFER" value="5m" hint="Expiry grace buffer" accent={4} />
  <MetricCard label="JWT_DEMO_EXPIRES_IN" value="2h" hint="Demo user TTL" accent={5} />
</MetricGrid>

### Data Flow

<PhaseTimeline>
  <PhaseMilestone phase="1" title="Server env" period="JWT_EXPIRES_IN=2h and 4 others" status="done" icon="Settings" accent={1} />
  <PhaseMilestone phase="2" title="SessionPolicyService" period="env → ms parsing + SSoT" status="done" icon="Server" accent={2} />
  <PhaseMilestone phase="3" title="Server consumer injection" period="JwtModule · Interceptor · OAuthService" status="done" icon="Workflow" accent={3} />
  <PhaseMilestone phase="4" title="Public API" period="GET /auth/session-policy" status="done" icon="Globe" accent={4} />
  <PhaseMilestone phase="5" title="Frontend fetch" period="1 call at app boot + DEFAULT fallback" status="done" icon="Monitor" accent={5} />
</PhaseTimeline>
...
<MetricGrid cols={4}>
  <MetricCard label="JWT TTL" value="2h" hint="7d → 2h" accent={1} />
  <MetricCard label="Cookie maxAge" value="Dynamic" hint="Fixed 1h → synced with JWT exp" accent={2} />
  <MetricCard label="Sliding threshold" value="1h" hint="5 min → 1 hour (50% of TTL)" accent={3} />
  <MetricCard label="Frontend timer" value="Policy fetch" hint="Fixed 65 min → synced with server policy" accent={4} />
</MetricGrid>
...
<Callout type="warn" title="Session Policy Sync Checklist">

...
category: challenge
order: 6
tldr: "The \"session is too short\" bug came down to four clocks scattered across four layers. A record of fixing it by unifying the SSoT — a structural shift, not a constant swap."
relatedAdrs: ["sprint-71", "sprint-65"]
---
...
JWT TTL is 2 hours, yet logout hits at 1 hour — four layers each kept their own clock.
</Problem>
...
<MetricGrid cols={4}>
  <MetricCard label="JWT TTL" value="7 days" hint="Production env (designed: 2h)" accent={1} />
  <MetricCard label="Cookie maxAge" value="1 hour" hint="Hardcoded constant" accent={2} />
  <MetricCard label="Sliding threshold" value="5 min" hint="Only refreshes within 5 min of expiry" accent={4} />
...
I decided to unify the four scattered clocks into a single SSoT.
</Decision>
...
<Result label="Result">
The change seen Before/After — the result of a structural shift, not a constant swap.
</Result>

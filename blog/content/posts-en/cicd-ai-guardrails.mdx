...
category: journey
order: 4
tldr: "The faster AI wrote code, the tighter verification had to be. A hands-on account of building a safety net for AI-generated code with 15 CI jobs — from security to quality, tests, builds, and GitOps locks."
...
What happens if you ship AI-written code as-is? The speed of generation turned the verification gap into risk.
</Problem>
...
I stacked the safety net step by step — 8 phases, 15 jobs, from security to a GitOps lock.
</Decision>
...
<Result label="Result">
The real incidents we hit, and the balance found between AI and CI.
</Result>
...
<Callout type="danger" title="Force-push banned">
`git commit --amend` + force-push disables `dorny/paths-filter` change detection. When change detection is breached, every subsequent check (tests, builds, scans) doesn't even execute. **A safe CI pipeline is always more important than a clean history.**
</Callout>
...
<Callout type="warn" title="Source repo ≠ GitOps repo">
ArgoCD syncs the aether-gitops repo. Changing only the source repo's manifests has zero effect on production. When changing manifests, **always update both repos together**.
</Callout>

...
category: journey
order: 1
tldr: "Building an MSA solo, I scaled AI agents from 8 to 12 and orchestrated them. The biggest takeaway was the difference between actually doing it and not."
...
Building alone is fast, but juggling 6 services left me without enough hands to keep productivity and consistency at once.
</Problem>
...
I scaled the agents from 8 to 12 and specialized their roles — shared rules in persona-base.md as SSoT, domain knowledge with each agent.
</Decision>
...
<EchelonMatrix>
  <EchelonMatrixRow echelon={1} label="Mission Critical" description="If the service breaks, everything stops. Uses the most powerful model (Opus)." accent={4}>
...
<Result label="Result">
Where this structure shone in practice (preventing a DB incident, catching security residue, preserving context) — and where it hit its limits.
</Result>
...
<Callout type="success" title="The moment an agent prevented a disaster">
Librarian's Expand-Contract rule forced a simple rename into a 3-phase deployment. Tedious, but zero downtime in production. Without the agent, I never would have followed this process.
</Callout>
...
<Pipeline>
  <PipelineStage label="Echelon 1" detail="Infrastructure · safety nets" accent={4} icon="Shield" />
  <PipelineStage label="Echelon 2" detail="Feature implementation · business logic" accent={1} icon="Workflow" />
  <PipelineStage label="Echelon 3" detail="UX · analysis · wrap-up" accent={3} icon="Sparkles" />
</Pipeline>

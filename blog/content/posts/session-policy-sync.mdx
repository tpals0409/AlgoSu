...
진단을 시작했습니다. 결론부터 말하면, **4개 레이어가 각자 다른 시계를 쓰고 있었습니다.**

> 이 글은 [AlgoSu 기술 블로그 시리즈](/posts/agent-orchestration-solo-dev) 7편입니다.
> 이전 글: [Sprint 3에서 67까지 — 혼자 만든 서비스의 성장 기록](/posts/sprint-journey)
...
<MetricGrid cols={4}>
  <MetricCard label="JWT TTL" value="7일" hint="운영 env (기획: 2h)" accent={5} />
  <MetricCard label="Cookie maxAge" value="1시간" hint="하드코딩 상수" accent={5} />
  <MetricCard label="Sliding 임계값" value="5분" hint="만료 5분 전에만 갱신" accent={5} />
  <MetricCard label="프론트 타이머" value="65분" hint="강제 로그아웃 판정" accent={5} />
</MetricGrid>
...
처음에는 각 하드코딩 값을 올바른 값으로 교체하는 초기안을 세웠습니다. 하지만 PM 피드백이 왔습니다.
...
<MetricGrid cols={3}>
  ...
  <MetricCard label="SESSION_REFRESH_THRESHOLD" value="1h" hint="Sliding 갱신 임계값" accent={2} />
  <MetricCard label="SESSION_HEARTBEAT_INTERVAL" value="10m" hint="프론트 heartbeat 주기" accent={3} />
  <MetricCard label="SESSION_TIMEOUT_BUFFER" value="5m" hint="만료 여유 버퍼" accent={4} />
  <MetricCard label="JWT_DEMO_EXPIRES_IN" value="2h" hint="데모 유저 전용 TTL" accent={5} />
</MetricGrid>

### 데이터 흐름

<PhaseTimeline>
  <PhaseMilestone phase="①" title="서버 env" period="JWT_EXPIRES_IN=2h 등 5종" status="done" icon="Settings" accent={1} />
  <PhaseMilestone phase="②" title="SessionPolicyService" period="env → ms 단위 파싱 + SSoT" status="done" icon="Server" accent={2} />
  <PhaseMilestone phase="③" title="서버 소비자 주입" period="JwtModule · Interceptor · OAuthService" status="done" icon="Workflow" accent={3} />
  <PhaseMilestone phase="④" title="공개 API" period="GET /auth/session-policy" status="done" icon="Globe" accent={4} />
  <PhaseMilestone phase="⑤" title="프론트 fetch" period="앱 부트 시 1회 + DEFAULT fallback" status="done" icon="Monitor" accent={5} />
</PhaseTimeline>
...
<MetricGrid cols={4}>
  ...
  <MetricCard label="Cookie maxAge" value="동적" hint="1h 고정 → JWT exp 연동" accent={2} />
  <MetricCard label="Sliding 임계값" value="1h" hint="5분 → 1시간 (TTL 50%)" accent={3} />
  <MetricCard label="프론트 타이머" value="정책 fetch" hint="65분 고정 → 서버 정책 연동" accent={4} />
</MetricGrid>
...
<Callout type="warn" title="세션 정책 동기화 체크리스트">

...
category: backend
order: 6
tldr: "\"세션이 너무 짧다\"는 버그의 핵심은 4개 레이어에 흩어진 4개의 시계였다. SSoT를 하나로 통일해 상수 교체가 아닌 구조 전환으로 해결한 기록."
...
<Problem label="문제">
JWT TTL은 2시간인데 1시간 만에 로그아웃 — 4개 레이어가 각자의 시계를 갖고 있었다.
</Problem>
...
<MetricGrid cols={4}>
  <MetricCard label="JWT TTL" value="7일" hint="운영 env (기획: 2h)" accent={1} />
  <MetricCard label="Cookie maxAge" value="1시간" hint="하드코딩 상수" accent={2} />
  <MetricCard label="Sliding 임계값" value="5분" hint="만료 5분 전에만 갱신" accent={4} />
...
<Decision label="결정">
흩어진 4개의 시계를 하나의 SSoT로 통일하기로 했다.
</Decision>
...
<Result label="결과">
Before/After로 본 변화 — 상수 교체가 아닌 구조 전환의 결과.
</Result>

...
category: ai-agent
order: 1
tldr: "혼자서 MSA를 만들며 AI 에이전트를 8명에서 12명으로 늘려 오케스트레이션했다. 직접 부딪혀 본 것과 안 해본 것의 차이가 가장 큰 수확이었다."
relatedAdrs: ["sprint-82", "sprint-116", "sprint-117"]
---
...
<Problem label="문제">
혼자 만들면 빠르지만, 6개 서비스를 오가며 생산성과 일관성을 동시에 유지할 손이 부족했다.
</Problem>
...
<Decision label="결정">
에이전트를 8명에서 12명으로 늘리고 역할을 전문화했다 — 공통 규칙은 persona-base.md SSoT로, 도메인 지식은 각자에게.
</Decision>
...
<EchelonMatrix>
  <EchelonMatrixRow echelon={1} label="Mission Critical" description="서비스가 깨지면 모든 것이 멈춥니다. 가장 강력한 모델(Opus)을 사용." accent={4}>
...
<Result label="결과">
실전에서 이 구조가 어디서 빛났고(DB 사고 방지·보안 잔재 차단·맥락 보존) 어디서 한계를 드러냈나.
</Result>
...
<Callout type="success" title="에이전트가 사고를 방지한 순간">
Librarian의 Expand-Contract 규칙이 단순 rename을 3단계 배포로 강제했습니다. 번거롭지만, 운영 중 다운타임 제로. 혼자였다면 절대 이 과정을 밟지 않았을 겁니다.
</Callout>
...
<Pipeline>
  <PipelineStage label="Echelon 1" detail="인프라 · 안전장치 확보" accent={4} icon="Shield" />
  <PipelineStage label="Echelon 2" detail="기능 구현 · 비즈니스 로직" accent={1} icon="Workflow" />
  <PipelineStage label="Echelon 3" detail="UX · 분석 · 마감" accent={3} icon="Sparkles" />
</Pipeline>

...
category: ai-agent
order: 10
tldr: "152 스프린트가 쌓이며 ADR·MEMORY·CLAUDE.md가 함께 부풀어 컨텍스트를 잠식했다. 슬라이딩 윈도우 알고리즘으로 agent 컨텍스트를 최적화해 7 스프린트 연속 회귀 0건을 달성한 기록."
relatedAdrs: ["sprint-82", "sprint-116"]
---

## 152번째 스프린트, 차곡차곡 쌓인 ADR들

오늘은 Sprint 152입니다. 처음부터 sprint별 ADR을 남겼던 건 아니었고, Sprint 62 무렵부터 매 스프린트의 결정과 회고를 한 파일로 묶기 시작했어요. 그때부터 지금까지 [`docs/adr/sprints/`](https://github.com/tpals0409/AlgoSu/tree/main/docs/adr/sprints) 아래에 `sprint-62.md`부터 `sprint-151.md`까지 90개 가까이 차곡차곡 쌓여 있습니다. 그 옆에는 [`CLAUDE.md`](https://github.com/tpals0409/AlgoSu/blob/main/CLAUDE.md)가 있고, [`.claude/commands/agents/`](https://github.com/tpals0409/AlgoSu/tree/main/.claude/commands/agents)에는 12개의 에이전트 페르소나가, 그리고 매 스프린트마다 갱신되는 `MEMORY.md`가 있어요.
...
<Problem label="문제">
쌓이는 문서가 매 세션 agent의 컨텍스트를 잠식하기 시작했다 — 꽉 찬 물통에 물을 더 붓는 일.
</Problem>
...
스프린트가 100을 넘어가면서, 그러니까 sprint별 ADR이 40개 가까이 쌓이던 시점부터 이상한 일들이 생기기 시작했어요. agent가 매 세션 시작할 때 컨텍스트로 들이미는 문서의 양이 너무 커졌습니다.
...
<Decision label="결정">
슬라이딩 윈도우 알고리즘에서 빌려온 단순한 규칙으로 컨텍스트를 다이어트하기로 했다.
</Decision>
...
문서는 무한히 쌓일 수 있습니다. sprint별 ADR은 90개에서 100개, 200개로 늘어날 거예요. 하지만 윈도우는 그대로 2입니다. agent가 매 세션 들고 가는 컨텍스트의 무게는 늘어나지 않아요. 늘어나는 건 영구 저장소의 깊이와 인덱스의 선명함뿐입니다.

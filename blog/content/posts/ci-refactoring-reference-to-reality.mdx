...
category: cicd
order: 8
tldr: "채널톡 CI 레퍼런스를 참고만 하다가 4 스프린트에 걸쳐 직접 CI를 리팩토링했다. Auto-merge·Composite Action·실측 규약까지, 참고와 실행의 거리를 좁힌 기록."
relatedAdrs: ["sprint-105", "sprint-107", "ADR-027"]
---
...
<Problem label="문제">
잘 만든 CI 레퍼런스는 많았지만, 우리 파이프라인에 그대로 들어맞지 않았다.
</Problem>
...
<Decision label="결정">
4 스프린트 로드맵으로 Auto-merge부터 실측 규약까지 단계적으로 직접 적용하기로 했다.
</Decision>

...
| **Agent-04** | **Sensei(분석가)** | AI Analysis Service (FastAPI) | Gemini API 연동, Circuit Breaker(cockatiel), RabbitMQ 소비자, 분석 결과 저장 |
...
| Agent | 작업 |
|-------|------|
| Sensei(분석가) | FastAPI AI Analysis Worker 구현, RabbitMQ 소비자, Gemini API 연동, Circuit Breaker(cockatiel — 실패율 50%, HALF-OPEN 30초), analysis_status = DELAYED fallback 처리 |

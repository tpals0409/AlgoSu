...
플랜 수립 전 아래를 확인하세요:
1. **이번 스프린트 하드 제약** (기술 부채·보안·CI 등): 최대 3개
2. **선결 리스크**: 작업 전 해소해야 할 미지의 사항 (ADR 충돌 여부 포함)
3. **관련 ADR**: `grep -r "관련 키워드" docs/adr/` 로 연관 ADR 번호 식별
4. **가정 명시**: 코드베이스 상태에 대한 전제 — 틀릴 경우 어떻게 대응할지

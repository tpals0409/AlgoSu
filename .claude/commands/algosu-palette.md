...
- **Palette 관련**: UI-1(토큰 리팩터링, 다크모드 개선), UI-2~UI-4(페이지별 컴포넌트), UI-6(WCAG 감사)
- **핵심 변경**: 태그 칩 그리드(Analytics), Monaco Editor 스타일링
- **결정 완료**: SVG 버블 차트 → 태그 칩 그리드 (직교 그리드 UI와 조화)
...
## 기술 스택
Next.js 14+ (App Router), Tailwind CSS v3, shadcn/ui, next-themes, Lucide React

...
- SSE 수신 (EventSource API): 최종 상태(DONE/FAILED) 수신 시 자동 종료
- UI 상태 3단계 실시간 표시 (제출완료 / GitHub동기화 / AI분석)
...
- **Herald 관련**: UI-1(httpOnly Cookie 전환, UUID URL), UI-2(스터디룸 네비게이션), UI-3(Monaco Editor 통합, 제출 UI), UI-4(대시보드 리뉴얼), UI-5(피어 코드 리뷰 UI)
- **핵심 변경**: localStorage 토큰 → httpOnly Cookie, Monaco Editor, 알림 패널 확장

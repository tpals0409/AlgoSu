...
import ProblemDetailErrorPage from '../error';
...
describe('ProblemDetailErrorPage', () => {
  ...
  it('오류 제목이 렌더링된다', () => {
    render(<ProblemDetailErrorPage error={mockError} reset={mockReset} />);
    expect(screen.getByText('문제 오류')).toBeInTheDocument();
  });

  it('오류 설명 메시지가 렌더링된다', () => {
    render(<ProblemDetailErrorPage error={mockError} reset={mockReset} />);
    expect(screen.getByText('문제 상세 정보를 불러올 수 없습니다.')).toBeInTheDocument();
  });

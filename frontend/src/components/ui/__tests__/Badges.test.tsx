...
describe('DifficultyBadge', () => {
  it('renders difficulty label', () => {
    renderWithI18n(<DifficultyBadge difficulty="GOLD" />);
    expect(screen.getByText(/Gold/)).toBeInTheDocument();
  });

  it('has aria-label with difficulty name', () => {
    renderWithI18n(<DifficultyBadge difficulty="DIAMOND" />);
    expect(screen.getByLabelText(/난이도: Diamond/)).toBeInTheDocument();
  });

  it('shows dot by default', () => {
    const { container } = renderWithI18n(<DifficultyBadge difficulty="SILVER" />);
    const dot = container.querySelector('[aria-hidden]');
    expect(dot).toBeInTheDocument();
  });

  it('hides dot when showDot=false', () => {
    const { container } = renderWithI18n(<DifficultyBadge difficulty="SILVER" showDot={false} />);
    const dot = container.querySelector('[aria-hidden]');
    expect(dot).not.toBeInTheDocument();
  });

  it('hides label when showLabel=false', () => {
    renderWithI18n(<DifficultyBadge difficulty="BRONZE" showLabel={false} />);
  ...
  it('renders tier number from level', () => {
    ...
    renderWithI18n(<DifficultyBadge difficulty="GOLD" level={12} />);
    expect(screen.getByText(/Gold 4/)).toBeInTheDocument();
  });

  it('applies custom className', () => {
    const { container } = renderWithI18n(<DifficultyBadge difficulty="PLATINUM" className="extra" />);
...
describe('LangBadge', () => {
  it('renders language text', () => {
    renderWithI18n(<LangBadge language="Python" />);
    expect(screen.getByText('Python')).toBeInTheDocument();
  });

  it('renders different languages', () => {
    renderWithI18n(<LangBadge language="C++" />);
    expect(screen.getByText('C++')).toBeInTheDocument();
  });

  it('applies custom className', () => {
    const { container } = renderWithI18n(<LangBadge language="Java" className="mono" />);
...
describe('CategoryBar', () => {
  ...
  it('renders category name', () => {
    renderWithI18n(<CategoryBar item={baseItem} />);
  ...
  it('calls onClick when clicked', () => {
    ...
    renderWithI18n(<CategoryBar item={baseItem} onClick={handleClick} />);
    fireEvent.click(screen.getByText('코드 품질'));
    expect(handleClick).toHaveBeenCalledTimes(1);
  });

  it('applies selected styling', () => {
    const { container } = renderWithI18n(<CategoryBar item={baseItem} selected />);
  ...
  it('renders warning color variant', () => {
    ...
    renderWithI18n(<CategoryBar item={warningItem} />);
    expect(screen.getByText('B')).toBeInTheDocument();
  });

  it('renders error color variant', () => {
    const errorItem: CategoryItem = { ...baseItem, color: 'error', grade: 'C', score: 45 };
    renderWithI18n(<CategoryBar item={errorItem} />);
    expect(screen.getByText('C')).toBeInTheDocument();
  });

  it('applies custom className', () => {
    const { container } = renderWithI18n(<CategoryBar item={baseItem} className="custom-bar" />);
...
describe('CategoryBar accessibility', () => {
  ...
  it('has role="button"', () => {
    renderWithI18n(<CategoryBar item={item} />);
  ...
  it('has aria-pressed=true when selected', () => {
    renderWithI18n(<CategoryBar item={item} selected />);
  ...
  it('triggers onClick on Enter key press', () => {
    const handleClick = jest.fn();
    renderWithI18n(<CategoryBar item={item} onClick={handleClick} />);

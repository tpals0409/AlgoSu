...
describe('CodePanel', () => {
  ...
  it('코드 라인들을 렌더링한다', () => {
    renderWithI18n(<CodePanel {...defaultProps} />);
  ...
  it('라인 클릭 시 onLineClick이 호출된다', () => {
    ...
    renderWithI18n(<CodePanel {...defaultProps} onLineClick={onLineClick} />);
  ...
  it('commentLines에 해당하는 라인에 댓글 인디케이터를 표시한다', () => {
    renderWithI18n(<CodePanel {...defaultProps} commentLines={[2]} />);
  ...
  it('하이라이트 정보가 라인에 적용된다', () => {
    ...
    renderWithI18n(<CodePanel {...defaultProps} highlights={highlights} />);
  ...
  it('selectedLine이 지정되면 해당 라인이 선택 스타일을 가진다', () => {
    renderWithI18n(<CodePanel {...defaultProps} selectedLine={2} />);
  ...
  it('commentLines에 있지만 selectedLine이 아닌 라인은 border-l-primary를 가진다', () => {
    renderWithI18n(<CodePanel {...defaultProps} commentLines={[1]} selectedLine={2} />);
    const line1 = screen.getByRole('button', { name: 'Line 1' });
  ...
  it('selectedLine이 null이면 스크롤하지 않는다', () => {
    ...
    renderWithI18n(<CodePanel {...defaultProps} selectedLine={null} />);
  ...
  it('selectedLine이 undefined이면 스크롤하지 않는다', () => {
    const scrollMock = jest.fn();
    Element.prototype.scrollIntoView = scrollMock;
    renderWithI18n(<CodePanel {...defaultProps} selectedLine={undefined} />);
    expect(scrollMock).not.toHaveBeenCalled();
  });

  it('존재하지 않는 selectedLine으로도 에러가 발생하지 않는다', () => {
    // 코드에 없는 라인 번호 (el이 null이 되는 경우)
    renderWithI18n(<CodePanel {...defaultProps} selectedLine={999} />);

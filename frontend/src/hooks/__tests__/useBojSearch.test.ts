/**
 * @file useBojSearch 단위 테스트
 */
import { renderHook, act, waitFor } from '@testing-library/react';
import { useState } from 'react';
import { useBojSearch } from '../useBojSearch';
import type { ProblemFormState } from '@/lib/problem-form-utils';

const mockSearch = jest.fn();
jest.mock('@/lib/api', () => ({
  solvedacApi: { search: (id: number) => mockSearch(id) },
}));

const defaultForm: ProblemFormState = {
  title: '',
  description: '',
  difficulty: 'EASY',
  weekNumber: '1',
  deadline: '',
  allowedLanguages: ['python'],
  sourceUrl: '',
  sourcePlatform: '',
};

function useTestHook() {
  const [form, setForm] = useState(defaultForm);
  const [fieldErrors, setFieldErrors] = useState<{ title?: string; weekNumber?: string; deadline?: string }>({});
  const boj = useBojSearch(setForm, setFieldErrors);
  return { form, fieldErrors, ...boj };
}

describe('useBojSearch', () => {
  beforeEach(() => {
    jest.clearAllMocks();
  });

  it('초기 상태가 올바르다', () => {
    const { result } = renderHook(() => useTestHook());

    expect(result.current.bojQuery).toBe('');
    expect(result.current.bojSearching).toBe(false);
    expect(result.current.bojError).toBeNull();
    expect(result.current.bojResult).toBeNull();
    expect(result.current.bojApplied).toBe(false);
  });

  it('유효하지 않은 문제 번호 시 에러를 설정한다', async () => {
    const { result } = renderHook(() => useTestHook());

    act(() => {
      result.current.setBojQuery('abc');
    });

    await act(async () => {
      await result.current.handleBojSearch();
    });

    expect(result.current.bojError).toBe('유효한 문제 번호를 입력해주세요.');
    expect(mockSearch).not.toHaveBeenCalled();
  });

  it('빈 문자열 시 에러를 설정한다', async () => {
    const { result } = renderHook(() => useTestHook());

    await act(async () => {
      await result.current.handleBojSearch();
    });

    expect(result.current.bojError).toBe('유효한 문제 번호를 입력해주세요.');
  });

  it('0 이하의 번호 시 에러를 설정한다', async () => {
    const { result } = renderHook(() => useTestHook());

    act(() => {
      result.current.setBojQuery('0');
    });

    await act(async () => {
      await result.current.handleBojSearch();
    });

    expect(result.current.bojError).toBe('유효한 문제 번호를 입력해주세요.');
  });

  it('소수점 번호 시 에러를 설정한다', async () => {
    const { result } = renderHook(() => useTestHook());

    act(() => {
      result.current.setBojQuery('1.5');
    });

    await act(async () => {
      await result.current.handleBojSearch();
    });

    expect(result.current.bojError).toBe('유효한 문제 번호를 입력해주세요.');
  });

  it('검색 성공 시 form을 업데이트한다', async () => {
    const info = {
      problemId: 1000,
      title: 'A+B',
      difficulty: 'BRONZE' as const,
      level: 1,
      sourceUrl: 'https://boj.kr/1000',
      tags: ['math'],
    };
    mockSearch.mockResolvedValue(info);

    const { result } = renderHook(() => useTestHook());

    act(() => {
      result.current.setBojQuery('1000');
    });

    await act(async () => {
      await result.current.handleBojSearch();
    });

    expect(result.current.bojResult).toEqual(info);
    expect(result.current.bojApplied).toBe(true);
    expect(result.current.bojSearching).toBe(false);
    expect(result.current.form.title).toBe('A+B');
    expect(result.current.form.difficulty).toBe('BRONZE');
    expect(result.current.form.sourceUrl).toBe('https://boj.kr/1000');
    expect(result.current.form.sourcePlatform).toBe('BOJ');
  });

  it('difficulty가 null이면 빈 문자열로 설정한다', async () => {
    const info = {
      problemId: 1000,
      title: 'A+B',
      difficulty: null,
      level: 0,
      sourceUrl: 'https://boj.kr/1000',
      tags: [],
    };
    mockSearch.mockResolvedValue(info);

    const { result } = renderHook(() => useTestHook());

    act(() => {
      result.current.setBojQuery('1000');
    });

    await act(async () => {
      await result.current.handleBojSearch();
    });

    expect(result.current.form.difficulty).toBe('');
  });

  it('검색 실패 시 에러를 설정한다 (Error 인스턴스)', async () => {
    mockSearch.mockRejectedValue(new Error('Not found'));

    const { result } = renderHook(() => useTestHook());

    act(() => {
      result.current.setBojQuery('99999');
    });

    await act(async () => {
      await result.current.handleBojSearch();
    });

    expect(result.current.bojError).toBe('Not found');
    expect(result.current.bojSearching).toBe(false);
  });

  it('검색 실패 시 기본 에러 메시지 (비-Error 객체)', async () => {
    mockSearch.mockRejectedValue('string error');

    const { result } = renderHook(() => useTestHook());

    act(() => {
      result.current.setBojQuery('99999');
    });

    await act(async () => {
      await result.current.handleBojSearch();
    });

    expect(result.current.bojError).toBe('검색에 실패했습니다.');
  });

  it('Enter 키 시 검색을 실행한다', async () => {
    mockSearch.mockResolvedValue({
      problemId: 1000,
      title: 'A+B',
      difficulty: 'BRONZE',
      level: 1,
      sourceUrl: 'https://boj.kr/1000',
      tags: [],
    });

    const { result } = renderHook(() => useTestHook());

    act(() => {
      result.current.setBojQuery('1000');
    });

    const preventDefault = jest.fn();
    await act(async () => {
      result.current.handleBojKeyDown({
        key: 'Enter',
        preventDefault,
      } as unknown as React.KeyboardEvent<HTMLInputElement>);
    });

    expect(preventDefault).toHaveBeenCalled();

    await waitFor(() => {
      expect(result.current.bojApplied).toBe(true);
    });
  });

  it('Enter 외 키는 무시한다', () => {
    const { result } = renderHook(() => useTestHook());

    const preventDefault = jest.fn();
    act(() => {
      result.current.handleBojKeyDown({
        key: 'Escape',
        preventDefault,
      } as unknown as React.KeyboardEvent<HTMLInputElement>);
    });

    expect(preventDefault).not.toHaveBeenCalled();
  });

  it('리셋 시 상태를 초기화한다', async () => {
    mockSearch.mockResolvedValue({
      problemId: 1000,
      title: 'A+B',
      difficulty: 'BRONZE',
      level: 1,
      sourceUrl: 'https://boj.kr/1000',
      tags: [],
    });

    const { result } = renderHook(() => useTestHook());

    act(() => {
      result.current.setBojQuery('1000');
    });

    await act(async () => {
      await result.current.handleBojSearch();
    });

    act(() => {
      result.current.handleBojReset();
    });

    expect(result.current.bojQuery).toBe('');
    expect(result.current.bojResult).toBeNull();
    expect(result.current.bojError).toBeNull();
    expect(result.current.bojApplied).toBe(false);
    expect(result.current.form.title).toBe('');
    expect(result.current.form.difficulty).toBe('');
    expect(result.current.form.sourceUrl).toBe('');
  });

  it('setBojError로 에러를 직접 설정/해제할 수 있다', () => {
    const { result } = renderHook(() => useTestHook());

    act(() => {
      result.current.setBojError('custom error');
    });
    expect(result.current.bojError).toBe('custom error');

    act(() => {
      result.current.setBojError(null);
    });
    expect(result.current.bojError).toBeNull();
  });
});

...
describe('[E2E] 프로그래머스 전 구간 플로우 — sourcePlatform 전달 검증', () => {
  ...
  describe('[C] AI 분석 프롬프트 — 플랫폼 맥락 주입 (Python subprocess)', () => {
    ...
    it('[C1] PROGRAMMERS → 프롬프트에 "프로그래머스 플랫폼" + "solution()" 포함', () => {
      if (!isPython310Plus()) {
      ...
      // Then: 프로그래머스 맥락 문구 포함
      expect(prompt).toContain('프로그래머스 플랫폼');
      expect(prompt).toContain('solution()');
      // BOJ 맥락 미포함
      expect(prompt).not.toContain('백준(BOJ)');
    });

    it('[C2] BOJ → 프롬프트에 "백준(BOJ) 플랫폼" 포함 (회귀)', () => {
      if (!isPython310Plus()) {
      ...
      expect(prompt).toContain('백준(BOJ) 플랫폼');
      expect(prompt).toContain('표준 입출력');

...
| 서비스 | 역할 | 기술 |
|---|---|---|
| Gateway | API Gateway + OAuth + JWT + SSE + Rate Limit | NestJS |
| Identity Service | 인증/권한 (마이그레이션 전용) | NestJS |
| Problem Service | 문제/마감 관리 + Dual Write | NestJS |
| Submission Service | 코드 제출 + Saga Orchestrator | NestJS |
| GitHub Worker | 비동기 GitHub Push | Node.js |
| AI Analysis Service | Claude API 코드 분석 + Circuit Breaker | Python/FastAPI |

## AI Agent TF 구성 (12개: Oracle + 11 Agent)
| Agent | 역할 | Tier |
|---|---|---|
...
| Agent-04 Sensei | AI Analysis Service (FastAPI + Claude API) | Tier 3 |
| Agent-05 Curator | Problem Service + Dual Write | Tier 2 |
| Agent-06 Architect | k3s + CI/CD + 모니터링 인프라 | Tier 2 |
| Agent-07 Librarian | PostgreSQL + TypeORM 마이그레이션 | Tier 1 |
| Agent-08 Herald | Next.js 페이지 로직 + SSE 수신 + 라우팅 | Tier 3 |
| Agent-09 Palette | 디자인 시스템 + shadcn/ui + Tailwind + 공통 컴포넌트 | Tier 3 |
| Agent-10 Scout | UI/UX 탐색 + 배포 감사 + 목업 대조 | Tier 3 |
| Agent-11 Scribe | 메모리/Skill/문서/문맥정리 전담 | — |
...
- **Circuit Breaker**: 자체 구현 (Claude API에 적용, 실패율 50% → OPEN / 30초 HALF-OPEN)
...
## UI 디자인 (v2, 2026-03-02)
- 메인 컬러: `#7C6AAE` (Light) / `#A08CD6` (Dark)
- 듀얼 테마 (Light/Dark), 47 CSS 변수
- 폰트: Sora + Noto Sans KR + JetBrains Mono
- 공통 컴포넌트 22종 (Badge, Button, Card, Input, DiffBadge, ScoreGauge 등)

...
## 원본 경로
`/root/AlgoSu/plan/UI ver2/` (10개 JSX, 253KB) *(원본: Mac Mini `/Users/leokim/Desktop/UI ver2/`)*
...
- **Prometheus + Loki 통합 규칙**: `/root/AlgoSu/docs/monitoring-log-rules.md` *(원본: Mac `/Users/leokim/Desktop/추가사항 문서/prometheus-loki-unified-convention.md`)*
...
### 확정 패턴 (UI Polish Sprint 완료, 2026-03-03)
- **금지 토큰**: `destructive`, `muted-foreground`, `text-foreground`, `bg-background`, `ring-ring`, `bg-bg2/bg3`, `primary-500/400/foreground`
- **DifficultyBadge**: `diff-*` 프리픽스 (difficulty-* 금지)
- **h1**: `text-[22px] font-bold tracking-tight text-text` 전 페이지 통일
- **필수 표시**: `text-[11px]` 고정 (text-[10px] 금지)
- **rounded**: `rounded-btn` (버튼), `rounded-card` (카드) — `rounded-lg` 직접 사용 금지
- **접근성**: focus-visible ring 필수, 장식 아이콘 `aria-hidden`, button `type` 명시

#### 15. 코드 에디터 UX 강화 (2026-03-04 완료)
- **자동완성**: BOJ 알고리즘 스니펫 29개 (기존 LANG_COMPLETIONS builtins/keywords 전부 제거)
  - Python 17, Java 5, C++ 5, JS/TS 각 1 — `wordBasedSuggestions: 'currentDocument'`로 변수명 자동완성
- **레이아웃**: 데스크톱 split-view (좌측 문제정보 420px sticky + 우측 에디터 flex-1), 반응형 높이 `calc(100vh - 16rem)`
- **풀스크린**: Maximize/Minimize 토글, Escape 해제 (suggest-widget.visible 체크), minimap 자동 활성화
- **모바일**: 자동완성/폰트 크기 토글 `hidden sm:flex`, 초기화/제출 버튼 축약, 헤더/푸터 패딩 조정
- **접근성**: sr-only 언어 선택 라벨, Ctrl+Enter 단축키 안내
- 검증: Scout 26/26 PASS, Palette 4.5/5

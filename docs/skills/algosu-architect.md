...
- GHCR 이미지 푸시 (`main-{git-sha}` 태그, `latest` 금지) — repo public (2026-03-04)
- aether-gitops 레포 태그 업데이트 → ArgoCD 자동 배포
- 모니터링: Prometheus(30s) + Grafana + Loki(72h)
- RabbitMQ, Redis k3s 배포
- Sealed Secrets 적용 (12개 SealedSecret 운영 중)
...
- **완료**: UI-1~UI-6, 3-1 Contract(Problem DB), OCI k3s v1.34.4 배포
- **OCI 현황**: k3s 16 Pod Running, Sealed Secrets 12개, ArgoCD manual sync, Traefik Helm
- **핵심 변경**: ArgoCD GitOps CD, MinIO(오브젝트 스토리지), k3d 16 Pod + OCI k3s 병행

## k8s Deployment 이름 매핑 (OCI k3s)
...
### 수동 배포 시 주의사항
- `kubectl rollout restart`는 이미지 재풀링 시도 → repo public이므로 GHCR 인증 불필요 (k3s pull secret은 유지)
- initContainer 이미지는 `kubectl set image`로 업데이트 안됨 → `kubectl patch --type='json'` 필요
- 상세 워크플로: `memory/algosu-deploy.md` § PM 라이브 테스트 핫픽스

...
> **적용 범위**: 모든 tracked `.md` 파일 — `git ls-files '*.md'` 전체
...
참조 frontend/README.md 갱신 필요
참조 .github/pull_request_template.md 갱신 필요
```

- 텍스트 내 bare `.md` 경로 패턴 추출 (link 외부 노출)
- **커버 prefix (Sprint 182 보강)**: `docs/`, `scripts/`, `blog/`, `frontend/`, `services/`, `infra/`, `.claude/`, `.github/` — `validateRef` 의 resolve 분기와 동일한 `REPO_ROOT_PREFIXES` SSOT 에서 생성. 이전엔 `docs/` 만 매칭하여 다른 top-level prefix 의 bare 참조는 깨져도 미검출되던 비대칭을 해소
- anchor 분리 동일

> **참고**: markdown link `[text](path)` 는 prefix 무관하게 모든 `.md` 경로를 검증한다. 본 룰은 link 가 아닌 **평문 노출** bare 참조에만 적용된다.
...
스크립트는 매 실행 inline fixture 8종을 검증한다 (`runRegressionFixtures()`).

- **Sprint 153 Phase G 5종 슬러그** (`docs/` prefix) — 디렉토리 이동/리네이밍 회귀 기준선
- **Sprint 182 non-docs prefix 3종** (`frontend/`, `services/`, `.github/` 가짜 경로) — broadened bare-path 룰이 `docs/` 외 prefix 도 검출하는지 dogfood

- ✅ 8/8 모두 broken 으로 검출 → exit 진행
- ❌ 검출 수 불일치 → **exit 2 self-test 실패** — 어떤 룰이 너무 좁아지거나 면제가 과도해진 시점에 즉시 인지

본 fixture 는 슬러그가 해소된 후에도 **회귀 차단 기준선** 역할을 한다.
...
- **2026-05-21** — Sprint 182: bare-path 비대칭 보강. `extractBareDocPaths` 가 `docs/` 만 매칭하던 갭을 `REPO_ROOT_PREFIXES` SSOT(8 prefix) 공유로 해소 — `validateRef` resolve 분기·markdown link 추출과 정렬. self-test fixture 5→8종(non-docs prefix dogfood). 보강 시점 corpus 신규 적발 0건(예방적·구조적 정합)

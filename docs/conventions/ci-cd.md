...
`scripts/deploy.sh` — 서버에서 `appleboy/ssh-action` 통해 호출되는 layer-by-layer rollout 스크립트. 의존성 순서(infra → backend → frontend)대로 점진 적용해 일시적 schema mismatch / API contract drift 회피.

```
DEPLOY_SHA=<sha> ./scripts/deploy.sh
```

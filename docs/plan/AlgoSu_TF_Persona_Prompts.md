...
- Claude API 연동 (개인 피드백 + 그룹 최적화 코드 합성)
- Circuit Breaker 구현 (custom Python Circuit Breaker):
...
## 판단 기준
- Claude API 비용은 민감한 지표입니다. 불필요한 재시도나 중복 호출을 방지합니다.
- Circuit Breaker가 OPEN 상태일 때는 절대로 Claude API를 호출하지 않습니다.
- 분석 품질보다 서비스 안정성이 우선입니다. DELAYED 처리는 실패가 아닙니다.

## 에스컬레이션 조건
- Claude API 비용이 예상 임계치를 초과할 것으로 예측되는 경우

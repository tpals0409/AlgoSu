# AlgoSu — MSA 전환 아키텍처 컨텍스트 문서 v3.1

> Architecture Context & Decision Record
> 2025년 기준 | 기술 스택 확정판 (v3: 페인포인트 해결 방안 전면 반영, v3.1: ArgoCD GitOps 전환)
...
1. [프로젝트 개요](#1-프로젝트-개요)
2. [절대 원칙](#2-절대-원칙)
3. [제출 방식 변경 (핵심 패러다임 전환)](#3-제출-방식-변경-핵심-패러다임-전환)
4. [마이크로서비스 설계](#4-마이크로서비스-설계)
5. [인프라 및 기술 스택](#5-인프라-및-기술-스택)
6. [신뢰성 설계](#6-신뢰성-설계)
7. [CI/CD 파이프라인](#7-cicd-파이프라인)
8. [모니터링 설계](#8-모니터링-설계)
9. [단계별 이행 계획](#9-단계별-이행-계획-roadmap)
10. [위험 요소 및 대응 방안](#10-위험-요소-및-대응-방안)
11. [페인포인트 해결 설계 (v3 신규)](#11-페인포인트-해결-설계-v3-신규)
12. [기술 결정 기록 (ADR)](#12-기술-결정-기록-adr-요약)
13. [배포 서버 연동](#13-배포-서버-연동)
...
[원칙 1] 자체 DB가 Single Source of Truth
  제출 데이터의 유일한 출처는 자체 DB다. GitHub/외부 서비스는 부차적 동기화 대상이다.

[원칙 2] 비동기 우선
  외부 API 호출(GitHub, AI)은 반드시 메시지 큐를 통한 비동기 처리. 사용자 응답 경로에 외부 의존성을 두지 않는다.

[원칙 3] 장애 격리
  개별 서비스 장애가 전체 시스템으로 전파되지 않도록 Circuit Breaker, DLQ, 타임아웃을 필수 적용한다.

[원칙 4] Forward-Only 마이그레이션
  DB 스키마 변경은 롤백하지 않는다. 코드만 rollback, 스키마는 유지. 파괴적 변경은 Expand-Contract 패턴.

[원칙 5] GitOps 단일 진실 공급원
  K8s 클러스터에 적용되는 모든 매니페스트는 aether-gitops 레포가 유일한 출처다.
  kubectl apply 수동 실행으로 클러스터 상태를 변경하는 것은 금지한다.
  매니페스트 변경은 반드시 aether-gitops 레포에 커밋 → ArgoCD 자동 sync를 통해 적용한다.
  예외: 긴급 장애 대응 시 kubectl 수동 개입 허용. 단, 사후 즉시 aether-gitops에 동기화.

---

## 3. 제출 방식 변경 (핵심 패러다임 전환)
...
### 3.2 신규 제출 플로우 (Saga Orchestration)
...
| CD | **ArgoCD (GitOps)** | aether-gitops 레포 감시 → 자동 배포 |
...
```
네임스페이스 구조:
  algosu     → AlgoSu 전체 서비스 (기존과 동일)
  argocd     → ArgoCD (배포 서버 관리, AlgoSu가 직접 접근하지 않음)
  monitoring → Prometheus, Grafana, Loki, Promtail
```

### 5.4 예상 리소스 배분 (24GB 기준)
...
|-----------|-----------|---------------|
| AI Analysis Service → Claude API | custom Python Circuit Breaker | `analysis_status = DELAYED` + "분석 지연 중" 알림 |
| GitHub Worker → GitHub API | Retry + Dead Letter Queue | `github_sync_status = FAILED` + 사용자 알림 |

**Circuit Breaker 동작 기준 (Claude API)**
...
아키텍처: GitHub Actions (CI) + ArgoCD (CD)
원칙: 앱 코드와 배포 매니페스트를 완전히 분리한다.

레포 구조:
  tpals0409/AlgoSu           → 앱 코드, Dockerfile, CI 워크플로우
  tpals0409/aether-gitops     → K8s 매니페스트 (ArgoCD가 감시하는 유일한 레포)

aether-gitops 레포 디렉토리:

```
aether-gitops/
├── algoso/                  ← AlgoSu 매니페스트 (Kustomize)
│   ├── base/
│   │   ├── kustomization.yaml
│   │   ├── namespace.yaml
│   │   ├── postgres.yaml
...
│   └── overlays/
│       └── prod/
│           └── kustomization.yaml   ← 이미지 태그가 여기서 관리됨
└── {future-tenant}/                 ← 향후 추가 테넌트
```

### 7.1 CI 파이프라인 (GitHub Actions — AlgoSu 레포)
...
단계:
  ...
  3. Quality Gate (ESLint/tsc/ruff)
  4. 단위 테스트 (Jest/pytest)
  5. Docker Build (QEMU/Buildx ARM64) → GHCR Push (main-{git-sha})
  6. Trivy Image Scan (CRITICAL → 파이프라인 차단)
  7. ★ aether-gitops 레포 이미지 태그 업데이트 (아래 상세)

PR 트리거: 1~4단계만 실행 (빌드/배포 없음)
```

### 7.2 CD 파이프라인 (ArgoCD — 배포 서버)

```
ArgoCD가 aether-gitops 레포를 주기적으로 감시 (polling 3분)
매니페스트 변경 감지 시:
  1. 자동 Sync 실행
  2. Rolling Update (서비스 의존성 순서 준수)
  3. Health Check (Liveness/Readiness Probe)
  4. 실패 시 자동 Rollback (이전 리비전)

AlgoSu는 ArgoCD를 직접 조작하지 않는다.
CI의 7단계(aether-gitops 태그 업데이트)만 수행하면 배포는 자동으로 진행된다.
```

### 7.3 CI 7단계 — aether-gitops 이미지 태그 업데이트

CI 파이프라인의 마지막 단계에서 aether-gitops 레포의 Kustomize overlay를 업데이트한다.

```yaml
# GitHub Actions 워크플로우 예시 (AlgoSu 레포 내)
- name: Update aether-gitops manifest
  env:
    IMAGE_TAG: main-${{ github.sha }}
    SERVICE: ${{ matrix.service }}       # 변경된 서비스명
  run: |
    git clone https://x-access-token:${GITOPS_TOKEN}@github.com/tpals0409/aether-gitops.git
    cd aether-gitops/algoso/overlays/prod

    # kustomization.yaml의 이미지 태그 업데이트
    kustomize edit set image ghcr.io/tpals0409/algosu-${SERVICE}=ghcr.io/tpals0409/algosu-${SERVICE}:${IMAGE_TAG}

    git add .
    git commit -m "deploy(algoso): ${SERVICE} → ${IMAGE_TAG}"
    git push
```

### 7.4 aether-gitops Kustomize overlay 예시

```yaml
# aether-gitops/algoso/overlays/prod/kustomization.yaml
...
images:
  - name: ghcr.io/tpals0409/algosu-gateway
    newTag: main-a1b2c3d4e5f6
  - name: ghcr.io/tpals0409/algosu-identity
    newTag: main-a1b2c3d4e5f6
  - name: ghcr.io/tpals0409/algosu-submission
    newTag: main-a1b2c3d4e5f6
  - name: ghcr.io/tpals0409/algosu-problem
    newTag: main-a1b2c3d4e5f6
  - name: ghcr.io/tpals0409/algosu-github-worker
    newTag: main-a1b2c3d4e5f6
  - name: ghcr.io/tpals0409/algosu-ai-analysis
    newTag: main-a1b2c3d4e5f6
  - name: ghcr.io/tpals0409/algosu-frontend
    newTag: main-a1b2c3d4e5f6
```
...
┌────────────────────────────────────────────────────────┐
│ 계층 1: ArgoCD 자동 롤백                                 │
│   Health Check 실패 → 이전 Sync 리비전으로 자동 복구       │
├────────────────────────────────────────────────────────┤
│ 계층 2: Git Revert 롤백                                  │
│   aether-gitops에서 이전 커밋으로 revert                  │
│   → ArgoCD가 감지 → 이전 이미지 태그로 자동 재배포         │
├────────────────────────────────────────────────────────┤
│ 계층 3: 수동 이미지 지정                                   │
│   aether-gitops의 kustomization.yaml에서                 │
│   특정 서비스의 newTag를 이전 SHA로 수동 변경              │
│   → 커밋/푸시 → ArgoCD 자동 sync                         │
└────────────────────────────────────────────────────────┘

DB 마이그레이션 원칙 (변경 없음):
  Forward-Only. 마이그레이션 자체 롤백 금지.
  코드만 rollback, 스키마 유지 (전방 호환 필수).
  파괴적 변경: Expand-Contract 2-Phase 패턴.
```

### 7.6 이미지 태그 규칙

```
Production:   main-{full-git-sha}    예: main-a1b2c3d4e5f6...
Dev (향후):   dev-{short-sha}        예: dev-a1b2c3d
Release (향후): v{semver}            예: v1.2.0

금지: latest 태그 사용, mutable 태그 배포
```

### 7.7 브랜치 전략

```
AlgoSu 레포: feature/{agent-id}/{기능명} → PR → main
aether-gitops 레포: CI 자동 커밋만 허용 (수동 커밋은 긴급 롤백 시에만)
```

### 7.8 GitHub Secrets 변경사항

```
[삭제 — 더 이상 필요 없음]
  K3S_HOST           ← SSH 직접 배포 폐지
  K3S_USER           ← SSH 직접 배포 폐지
  K3S_SSH_KEY        ← SSH 직접 배포 폐지

[추가]
  GITOPS_TOKEN       ← aether-gitops 레포에 push할 수 있는 GitHub PAT
                       scope: repo (aether-gitops에 대한 write 권한)

[유지]
  GITHUB_TOKEN       ← GHCR push, gitleaks, Trivy 인증 (자동)
  DISCORD_WEBHOOK_DEPLOY     ← 배포 성공 알림 (선택)
  DISCORD_WEBHOOK_EMERGENCY  ← 배포 실패 알림 (선택)
```
...
| **Phase 2** | AI Analysis 서비스 분리 | FastAPI Worker 추출, RabbitMQ 도입, 제출-분석 비동기 디커플링, Circuit Breaker 적용, SSE 상태 노출 구현 |
...
| 운영 복잡도 | GitHub Actions CI + ArgoCD GitOps 자동 배포로 수동 개입 제거 |
...
| AI 비용 폭증 | Claude API Circuit Breaker + 호출 상한선 + Grafana 사용량 모니터링 |
...
### 11.3 Dual Write Reconciliation
...
### 11.5 Auto-save (브라우저 종료 시 코드 소실 방지)
...
| Circuit Breaker | custom Python Circuit Breaker (Claude만) | nestjs-resilience, 전체 적용 | 경량, GitHub Worker는 Retry로 충분 |
| Secret 관리 | Sealed Secrets | HashiCorp Vault | Vault 운영 리소스 부담 제거 |
| SLO 기준 | MVP 여유 기준 | 엄격한 기준 | OCI Free Tier 단일 노드 환경, 초기 안정화 우선 |
| CD 방식 | **ArgoCD (GitOps)** | GitHub Actions SSH 배포 | 앱/매니페스트 분리, 자동 롤백, Drift 감지 |
...
```
AlgoSu는 OCI Free Tier ARM 서버에서 k3s 클러스터로 운영된다.
배포 서버에는 ArgoCD가 설치되어 있으며, GitOps 방식으로 배포를 관리한다.

AlgoSu가 알아야 할 것:
  - aether-gitops 레포에 매니페스트를 커밋하면 자동 배포된다.
  - 배포 서버의 내부 구조를 알 필요 없다.
  - kubectl로 서버에 직접 접근하여 상태를 변경하지 않는다.

AlgoSu가 알 필요 없는 것:
  - 배포 서버의 관리 체계
  - ArgoCD 외의 서버 내부 컴포넌트
  - 다른 테넌트의 존재
```

### 13.2 AlgoSu ↔ 배포 서버 인터페이스

```
AlgoSu와 배포 서버 사이의 유일한 인터페이스는 aether-gitops Git 레포다.

  AlgoSu CI ──(git push)──► aether-gitops/algoso/ ──(ArgoCD 감시)──► k3s 클러스터

AlgoSu가 하는 것:
  1. 앱 코드 빌드 → GHCR에 이미지 push
  2. aether-gitops/algoso/overlays/prod/kustomization.yaml 태그 업데이트
  3. 끝. 이후는 배포 서버가 자동 처리.

AlgoSu가 하지 않는 것:
  - SSH로 서버 접속
  - kubectl 직접 실행
  - ArgoCD API 호출
  - aether-gitops의 algoso/ 외 디렉토리 접근
```
...
```
소유권:
  aether-gitops/algoso/base/        → AlgoSu Architect(Agent-06)가 관리
  aether-gitops/algoso/overlays/    → CI 자동 커밋 (이미지 태그만)
  aether-gitops의 algoso/ 외 경로   → 접근 금지

매니페스트 변경 절차:
  K8s 리소스 추가/수정 시:
    1. AlgoSu 레포에서 매니페스트 작성/수정
    2. aether-gitops/algoso/base/ 에 반영
    3. PR → 리뷰 → merge
    4. ArgoCD가 자동 감지 → 클러스터에 적용

  이미지 태그 업데이트 시:
    1. CI 파이프라인이 자동으로 aether-gitops에 커밋
    2. 수동 개입 불필요

커밋 메시지 규칙 (aether-gitops):
  이미지 태그 변경:  deploy(algoso): {service} → main-{sha}
  매니페스트 변경:   infra(algoso): {변경 내용 요약}
```

### 13.4 Sealed Secrets 관리

```
Sealed Secrets는 aether-gitops 레포에서 관리한다.

  aether-gitops/algoso/base/sealed-secrets/
    ├── sealed-gateway-secrets.yaml
    ├── sealed-identity-service-secrets.yaml
    └── ... (기존 10개 동일)

신규 Secret 추가 절차:
  1. 로컬에서 kubeseal로 암호화
  2. aether-gitops/algoso/base/sealed-secrets/ 에 추가
  3. kustomization.yaml resources에 등록
  4. PR → merge → ArgoCD 자동 적용

평문 Secret을 Git에 커밋하는 것은 절대 금지한다.
```
...
> **문서 버전**: v3.1
> **최종 수정**: 2026년
> **변경 이력**: v1 초안 → v2 설계 결정 반영 → v3 페인포인트 7개 해결 방안 전면 반영 → v3.1 ArgoCD GitOps 전환 + 절대 원칙 추가 + 배포 서버 연동

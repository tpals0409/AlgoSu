...
try {
  await this.claudeClient.analyze(code);
} catch (error: unknown) {
  if (error instanceof ClaudeApiError) {
    this.logger.error('Claude API 호출 실패', { submissionId, error });
    throw error;
  }
...
# ✅ 구체적인 예외 타입 명시
try:
    result = await claude_client.analyze(code)
...
logger.error("Claude API 오류", extra={"submission_id": str(submission_id), "status_code": 500})

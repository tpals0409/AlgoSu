...
docs/adr/
├─ README.md                              ← 본 문서
├─ ADR-{NNN}-{slug}.md                    ← 영구 ADR (11개)
├─ topics/
│  └─ {topic-slug}.md                     ← 토픽 ADR (1개)
└─ sprints/
   └─ sprint-{NN}.md                      ← 회고형 sprint ADR (195개, Sprint 62~261 일부 결번[256~259 등] + 통합 40·48·51)
```
...
| [ADR-031](./ADR-031-feedback-github-issue-sync.md) | 사용자 피드백 → GitHub 이슈 자동 동기화 (DB 경계 외부 반출) | 수락 | Sprint 261 |

> ADR-004 ~ 023은 미작성 (sprint별 회고형 ADR로 통합되었거나 본 형식으로 승격 예정). 새로 영구 ADR을 작성할 때는 **다음 빈 번호(ADR-032)** 부터 사용.
...
`sprints/sprint-{NN}.md` — Sprint 62 ~ 255 매 sprint 회고. 신규 sprint 종료 시 `/stop` 워크플로우가 자동 생성/갱신 (KR + EN 동시 작성).

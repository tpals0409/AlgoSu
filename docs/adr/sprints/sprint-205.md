...
- **R1** `019e6b9c-2fb4-7030-b74d-797449a8a33b` — Critical/High **0** + P2 **2** + P3 **0**
  - P2-1: `_base.md:51` 금지 조항 조기 제거 — Sprint 204 repo-side 삭제 후에도 다른 머신·CI 체크아웃에 `discord-send.sh` 잔존 가능 + BOT_TOKEN revoke 보류 상태에서 토큰 유효 가능성. 에이전트 억제 가드 부재 상태로 판단.
  - P2-2: `claude-tools.md:65` Phase 4 행 '완전 종결' 표현이 Sprint 205 ADR §D0·D1의 (a) 보류·(b) 미완료 상태와 모순.
  - 본 commit에서 두 항목 해소: `_base.md:51` 금지 조항 재보존(조건부 — 다른 머신 정리 완료 + 토큰 revoke 완료 확인 후 제거) + `claude-tools.md:65` '시크릿 노출 위험 종결 **미완료**'로 시제 정합.
- **placeholder 회귀 차단 결정 준수** (Sprint 204 R4): R1까지 ADR §Critic 라운드 영속화. R2+ 결과는 sprint-window/메모리에만 기록 — ADR 추가 commit 회피.

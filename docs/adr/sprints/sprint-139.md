...
PR #181 머지 직후 사용자 시각 검증에서 캘린더 회귀 3건 발견:

1. **ko locale 미적용** — 영문 월 ("April 2026") + 영문 요일 ("Su Mo Tu We Th Fr Sa") 표시
2. **요일 헤더 grid 깨짐** — `Su`만 첫 컬럼 위에, 나머지 `MoTuWeThFrSa`는 한 덩어리로 4번째 컬럼 위에 붙음
3. **모달 좌측 치우침** — 캘린더가 가운데 정렬 안됨

### 근본 원인
react-day-picker **v9.14.0**이 설치되어 있으나 `frontend/src/components/ui/calendar.tsx`가 **v8 className 키**를 사용 중이라 v9 표준 키와 매칭되지 않아 8개 className이 모두 무효화되며 default flex layout으로 폴백.
...
### 수정
- v9 className 키 매핑 갱신
- `locale` prop 기본값 `ko` (`react-day-picker/locale`에서 import — date-fns/locale 재export)
- `weekdays`/`week`를 `grid grid-cols-7`로 강제 → 7칸 균등 분할
- 캘린더 외각 `mx-auto w-fit` → 모달 가운데 정렬
- AlgoSu 디자인 토큰 (`text-text-3`, `bg-primary-soft` 등) 적용
- `selected`/`today`/`range_*` 상태를 v9 data attribute selector(`data-[selected-single]`/`data-[range-*]`)로 적용

### 검증
- typecheck pass / lint warnings only / 1348 tests pass (회귀 0건)
...
## Sprint 140 이월
- Oracle `oracle-spawn.sh` PATH 환경 점검 (Sprint 139 발생, P1)
- 의존성 major 업그레이드 wrapper 호환성 점검 (Sprint 139 follow-up 발생 — PR pre-flight checklist 추가)
- en locale 동적 매핑 (next-intl locale ↔ react-day-picker locale 연결 — Sprint 139 follow-up 비범위)
- 누적 시드 4건 유지:

...
## Phase 1 후속 (2026-06-24)

정식 전환 직후 **이식 충실도 점검**에서 디스패치 기준 공백 1건을 발견·봉합했다. 모두 `~/.hermes/` 프로파일 자산(SOUL.md·스킬) 변경이며, repo 정본 변경은 본 ADR 보강 1건뿐.

- **이식 충실도 전수 점검**: `.claude/commands` 원본 vs Hermes 프로파일 스킬 대조. `_base` 오라클 프로토콜은 별도 스킬이 아니라 12 에이전트 스킬 **본문에 내장**됨을 확인(올바른 패턴). 잔존 `.claude/commands` 언급은 전부 frontmatter `source:` 출처 표기일 뿐 런타임 **글루 누출 0건**. 라이프사이클은 결정 5(하이브리드) 준수.
- **보조 스킬 2건 이식**: 미이식이던 `algosu-adr-blog`(ADR→블로그 변환)·`algosu-review`(코드 리뷰 체크리스트)를 정식 이식 → `algosu/` 카테고리 14→**16** 스킬(실측). `algosu-review`는 critic 흡수 대신 **상호 보완 분리** 채택(본 스킬=도메인·Gatekeeper 공용 자체 점검, `algosu-agent-critic`=Codex 외부 교차 리뷰).
- **디스패치 매트릭스 보강(공백 봉합)**: 12 페르소나 스킬 이식 시 빠져 있던 **Hermes 네이티브 디스패치 결정 기준**을 상시 주입 페르소나 `SOUL.md`에 `## Orchestration & Dispatch` 6규칙으로 신설(49줄). 원본 `algosu-oracle.md`의 디스패치 섹션은 전부 tmux 기반(결정 1로 폐기)이라 대체 매트릭스가 **미이식 공백**이었음 → 봉합. 직접처리/`delegate_task` 위임 경계·12 에이전트 Tier 도메인 라우팅·Critic 게이트(`codex review -c model="gpt-5.5"`)·라이프사이클·결정 우선순위(안정성>속도>완성도) 포함. 글루 누출 0건 재검증.
- 교훈: **"본체는 시스템 프롬프트가 흡수"라는 1차 판정에도 디스패치 기준만은 실측 공백이었다** — 상시 적용 행동 규칙은 로드형 스킬이 아니라 상시 주입(SOUL.md)에 둬야 매 턴 작동.

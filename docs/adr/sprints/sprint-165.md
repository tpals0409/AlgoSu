...
- `.github/workflows/ci.yml:895` "Upload Trivy SARIF" step 의 `if` 조건 확장:
  ```yaml
  if: >
    always() &&
...
- SARIF 업로드만 same-repo PR + main 으로 제한 → fork/Dependabot noisy red 차단
...
- **origin/main**: `ecfe954` → **`<TBD>`** (PR #290 squash merge 후 sprint-window.md 갱신)
- **commits**: Phase A `b8c6918` + Phase B `1f9c364` + Phase C `<TBD>` + Phase D `<TBD>` (총 4 commit → squash merge)

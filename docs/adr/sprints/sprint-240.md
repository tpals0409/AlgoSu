...
## 작업 요약 (Architect + Postman + Scribe, 총 3 commit)

### S-6 — `encryption-key-rotation.md` 신규 작성 (Architect)

- **근거 파일 직접 Read**: `services/gateway/src/auth/oauth/token-crypto.util.ts`(암호화), `services/github-worker/src/worker.ts:384-398`(fallback 경로), `infra/sealed-secrets/sealed-secrets-template.yaml:58,164`(SealedSecret 2곳), `docs/runbook/key-rotation.md`(kubeseal 컨벤션).
...
- **§3 2-Secret 동시 교체**: gateway-secrets + github-worker-secrets **단일 aether-gitops 커밋** 동시 교체 원칙 명기. 기존 secret 교체라 Sprint 236의 신규 secret 2-commit ordering(추가→검증→전환)과는 다른 케이스임을 구분. 전체 키 포함 주의(`key-rotation.md §3` 동일 경고).
- **§4 검증 게이트 4종**: ① sha256 일치 확인(평문 노출 없이) ② 신규 GitHub 연동 positive ③ 기존 토큰 `GITHUB_APP_FALLBACK` 로그 = 정상 신호 ④ 에러율·DLQ 무변동(닫힘 증명).
- **§5 롤백**: 구 키로 양 Secret 단일 커밋 복원 + ArgoCD sync.
- **§6 사후 조치**: 평문 키 삭제, 로테이션 기록 테이블.
- **§7 관련 문서**: key-rotation.md, github-token-relink.md, sealed-secrets-template.yaml, ADR-030.

### Q-3 — `dlq-redrive.md` 신규 작성 (Postman)
...
- **§3 redrive 절차**: 권장 = **dynamic shovel**(DLQ → 원본 exchange + routing key, 메시지 유실 없음, 완료 후 shovel 삭제) / 대안 = rabbitmqadmin get→publish 수동 루프(소량일 때). 워커별 정확한 exchange·routing key 명기.
...
- Critic 교차 리뷰 (Codex, `--base 924c650`) — 결과는 머지 직전 표기.
...
신규패턴: **예상 동작 명기 런북 패턴**(fallback 경로·복호화 실패를 "정상 신호"로 명시), **근본 원인 선행 게이트 패턴**(redrive §1에 reason별 "redrive 해도 되는 조건" 분기).

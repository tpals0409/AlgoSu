...
| **S-3** | CSP allows `script-src 'unsafe-inline'` (required by AdSense + Next.js inline scripts) — SSOT is the aether-gitops Traefik middleware; the gateway middleware mirrors the same value. A nonce/strict-dynamic migration requires Next.js build cooperation | `services/gateway/src/common/middleware/security-headers.middleware.ts:32`, aether-gitops `algosu/base/ingress.yaml` | Feasibility spike for nonce-based CSP — verify Next.js App Router nonce support + AdSense dependency before deciding | **Sprint 243 spike result: deferred.** Monaco Editor's `unsafe-eval` dependency cannot be resolved with nonce, so the real security gain is marginal. The architectural cost of converting all routes to force-dynamic (loss of ISR/CDN caching) and the ADR-029 Traefik SSOT multi-repo overhead outweighs the benefit — re-evaluate after removing `unsafe-eval` (Monaco replacement or wasm isolation). Re-evaluate triggers: ① Monaco replacement complete ② decision to convert all routes to SSR ③ ADR-029 Traefik SSOT re-discussion |
...
| **S-6** | `GITHUB_TOKEN_ENCRYPTION_KEY` is triple-managed in three SealedSecrets (gateway/github-worker/identity-service) — no documented rotation procedure (mismatch causes decryption failure). The audit identified two locations; Sprint 240 Critic R1 caught the additional identity usage | `infra/sealed-secrets/sealed-secrets-template.yaml:58,88,167` | Write a key-rotation runbook (simultaneous 3-key replacement procedure + verification gate) | Sprint 240 ✅ |
...
| Backlog | Deferred (user-confirmed) | Q-4 shared libs spike (build-pipeline impact investigation required first), S-3 full rollout (re-evaluate after removing Monaco `unsafe-eval`) |

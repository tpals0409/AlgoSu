...
tldr: "Sprint 2 of the ADR-030 remediation roadmap. Two undocumented operational gaps closed with zero code changes — runbook-only. S-6: GITHUB_TOKEN_ENCRYPTION_KEY rotation runbook (encryption-key-rotation.md) — dual-key unsupported means rotation invalidates all existing encrypted tokens, but worker.ts fallback path (GitHub App Installation Token) keeps the service live. 2-Secret (gateway-secrets + github-worker-secrets) simultaneous replacement in a single aether-gitops commit + 4-point verification gate (sha256 match / positive / fallback / closed). Q-3: DLQ redrive runbook (dlq-redrive.md) — topology table (submission.events → 2 queues → 2 DLQs), 5-reason table, root-cause-removal pre-gate (reason-based branch), dynamic shovel recommended + rabbitmqadmin fallback, idempotency caveat (github-worker redis TTL 1 h). 4 cross-ref updates + ADR-030 S-6/Q-3 Sprint 240 ✅. Verification: 5 ADR gates + doc-ref-lint PASS."
---

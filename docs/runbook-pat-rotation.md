...
### 2.3 서버: gh CLI 재인증 (device flow)

> **환경 전제**: 본 런북은 OCI ARM 서버(headless, GUI 브라우저 없음)에서 실행되는 것을 전제로 한다. `gh auth login --web`은 headless 환경에서 자동으로 **device flow**로 동작하여 URL + 8자리 코드를 터미널에 출력한다. 서버에서 브라우저를 열 필요 없이, **PM이 자신의 로컬 기기(노트북/데스크톱) 브라우저**에서 해당 URL을 접속해 코드를 입력하면 된다.

**주의**: gh CLI가 이미 `github.com`에 로그인되어 있는 경우 `gh auth login`은 `"already logged in, re-authenticate?"` 대화형 프롬프트를 띄워 비대화형 실행에서 stuck된다. **반드시 logout 선행**.

```bash
# (1) 기존 세션 제거 — hosts.yml의 기존 엔트리 정리 (backup은 §2.2에서 이미 완료)
gh auth logout --hostname github.com

# (2) device flow 로그인 시작
gh auth login --hostname github.com --git-protocol https --web \
  --scopes "admin:org,repo,workflow"
```

`gh auth login` 실행 시 터미널에 다음 형식으로 출력된다:
...
- Claude Code 환경에서는 Bash tool의 stdout으로 이 텍스트가 Oracle에게 반환됨 → Oracle이 PM에게 URL + 코드를 복사해 보여줌
- **PM 로컬 브라우저**: `https://github.com/login/device` 접속 → 8자리 코드 입력 → 로그인 확인 → 스코프 확인 후 Authorize
- 승인 완료 시 서버 측 `gh auth login` 프로세스가 자동으로 토큰을 받아 `/root/.config/gh/hosts.yml` 2 엔트리에 원자적 기록
- **새 토큰 문자열은 터미널에 출력되지 않음** (gh CLI secure storage)

**비대화형 Bash tool 제약**: `gh auth login`은 "Press Enter to open github.com in your browser..." 지점에서 stdin을 기다린다. Bash tool은 대화형 입력 주입이 제한적이므로 다음 중 하나를 사용:

```bash
# 방법 1: stdin에 Enter 주입
printf '\n' | gh auth login --hostname github.com --git-protocol https --web \
  --scopes "admin:org,repo,workflow"

# 방법 2: timeout으로 감싸되 device code 출력 이후 프로세스는 백그라운드 유지
# (run_in_background 또는 Monitor tool 사용 — Claude Code 환경 한정)
```
